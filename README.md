...
- 2017년 부터 웹스퀘어5의 패턴들이 확 달라졌음. 그전버전은 웬만하면 참고하지 말 것.
...
```     
  ...
  - min- : 이 prefix는 width 나 height 를 보정해주는 역할
  ...
  - WFrame 을 사용했을 때 자식과 부모의 id가 중복되지 않게 하는 scope 의 기능이 있다.
    ...
    - **중요!! 자식페이지랑 부모페이지에 id 에 대한 접근을 할 때 자체적으로 바꿔준 id로 써주는게 아니라 고유로 쓴 id로 접근하면 된다.**
...
- getvalue 로 직접 값을 설정하기보다 ref 를 적극적으로 활용할것!!!(ref 또는 setNodeSet())

- GridView는 실질적으로는 그저 display 역할 뿐
  - **매우중요!! 얘는 DataList 라는 짝꿍이 있다. 그냥 DataList 가 전부다.**
